function sigRecon = TGV_NPK(sigNoisy,numIter,padNum,alpha,beta,theta1,theta2)
...
% Syntax adapted for matlab4DSTEM package by Nathanael Kazmierczak - 2020
% June.
...
% NPK mod to make this work better with nans
% Assume that the tear mask is all the way through the image.
nanmask = isnan(sigNoisy);
nanboundary = boundarymask(nanmask) &~ nanmask;
[r,c] = size(nanmask);
for i = 1:r
    boundary_cols = find(nanboundary(i,:));
    [firstcol] = min(boundary_cols);
    [secondcol] = max(boundary_cols);
    firstval = sigNoisy(i,firstcol);
    secondval = sigNoisy(i,secondcol);
    width = secondcol - firstcol;
    sigNoisy(i,firstcol:round(firstcol+width/2)) = firstval;
    sigNoisy(i,round(secondcol-width/2):secondcol) = secondval;
end
sigNoisy(isnan(sigNoisy)) = 0;

f0=padarray(sigNoisy,[padNum,padNum],'symmetric');
f0=double(f0);
[m,n]=size(f0);

paddednanmask = padarray(nanboundary | nanmask,[padNum,padNum],'symmetric');
...
if flagPlot == true
    % NPK 06/18/2020: add omitnan flag for processing datasets with a tear
    % mask.
    sigMean = median(sigNoisy(:),'omitnan');
    sigRMS = sqrt(mean((sigNoisy(:) - sigMean(:)).^2,'omitnan'));
end
...
for step = 1:numIter
    ...
    % NPK mod: keep everything on boundary constant.
    u(paddednanmask) = f0(paddednanmask);
