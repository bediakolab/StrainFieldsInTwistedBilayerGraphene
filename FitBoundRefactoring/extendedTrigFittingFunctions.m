function [ pred_vals, J ] = extendedTrigFittingFunctions( DSC_guess, scaling_constant, bound_handling_flag, third_fourth_disk_flag )
...
if nargout > 1
     J(:,1) = 2*scaling_constant'.*cos(mul.*mags).*sin(mul.*mags).*mul.*mat(:,1);
     J(:,2) = 2*scaling_constant'.*cos(mul.*mags).*sin(mul.*mags).*mul.*mat(:,2);
end
