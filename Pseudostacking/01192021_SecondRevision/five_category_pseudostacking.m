% five_category_pseudostacking.m
%
...
ps_storage = zeros(n_datasets,5);  % for the five pseudostacked regions under the new partitioning scheme.
...
plot(sorted(:,1),sorted(:,5),'-o');
plot(sorted(:,1),sorted(:,6),'-o');

xlabel('Moire twist angle (degrees)');
ylabel('Pseudostacking %');
legend('AB reconstruction regime','AA reconstruction regime','AA inner','AA outer','Pure SP','Pure AB','AB/SP transitional');
set(gca,'FontSize',14)
