% get_pseudostacking_trends_withnewdata_andnewcriterion_noTGV.m
%
...
% % %     SPbufferdist = SP_bufferwidth_dict(idx,2);
    ...
    % No strain calculation needed because using the raw data.
...
% %     m4.computeStrainMaps3(trim_value,pre_filterstruct,TGVid,calibration_method,divide_by_two_for_intralayer,sign_convention,trim_tear_value,SPdirection);
    ...
    use_annealed = 0;  % going from raw data
    croprange = [];  % we don't need any cropping
    % This filter doesn't work on DS26, but should be okay for most others.
...
%         h.HandleVisibility = 'on';
        if mod(dataset_id(i),1) ~= 0
            ...
            excel_name = sprintf('Pseudostacking_DS_%dS2.csv',dsid_to_use);
        else
