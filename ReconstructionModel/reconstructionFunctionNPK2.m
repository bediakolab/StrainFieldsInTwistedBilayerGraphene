function [ xy_lat, rotfield ] = reconstructionFunctionNPK2( AA_angle, AA_distance, AB_angle, AB_buffer_distance, AB_smooth_distance, xy_lat, cellDimXY, plotrotfield, gamma, corner_angle_deg )
...
%
% Modified to produce correct soliton wall boundary conditions by
% interpolating the rotation angle around the AB centroid with the
% rotation angle parallel to the AB plate boundary.
% 
...
side_length = sum((vertices(2,:) - vertices(3,:)).^2).^0.5;
...
for i = 1:nLat1
    ...
    [ xy_lat ] = rotateOnAB_v2( xy_lat, xshift, yshift, anglemat_1, gamma, is_right_facing, corner_angle_deg, side_length );
end
for i = 1:nLat2
    if mod(i,10) == 0
    ...
    [ xy_lat ] = rotateOnAB_v2( xy_lat, xshift, yshift, anglemat_2, gamma, is_right_facing, corner_angle_deg, side_length );
end
