% get_SP_strain_postColin_soliton1_originaldata_hasmaxshear.m
%
...
maximum_shear_avs = vals_storage(:,8);
maximum_shear_stderrs = stderrs_storage(:,8);

figure
errorbar(twist_angle_data,100*2*deg2rad(fixed_body_rotation_avs)',2*2*fixed_body_rotation_stderrs',2*2*fixed_body_rotation_stderrs',xneg,xpos,'o');
...
hold on
errorbar(twist_angle_data,syx_avs',syx_stderrs',syx_stderrs',xneg,xpos,'o');
hold on
errorbar(twist_angle_data,principal_strain_avs',principal_strain_stderrs',principal_strain_stderrs',xneg,xpos,'o');
...
% New plot
figure;
errorbar(twist_angle_data,sxy_avs',2*sxy_stderrs',2*sxy_stderrs',xneg,xpos,'o');
hold on
errorbar(twist_angle_data,syx_avs',2*syx_stderrs',2*syx_stderrs',xneg,xpos,'o');
errorbar(twist_angle_data,maximum_shear_avs',2*maximum_shear_stderrs',2*maximum_shear_stderrs',xneg,xpos,'o');
xlabel('Twist angle (degrees)');
ylabel('Strain %')
set(gca,'FontSize',14);
legend('sxy (AA shear)','syx (AB shear)','Maximum shear strain (gamma max)');
title('Strain variation with twist for 1 layer; 95% CI y-axis errorbars');
